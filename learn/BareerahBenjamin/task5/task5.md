...
| 演示接收地址 | `0xE83B1DCF8F9F3765DAbd34555f39240a14f5AcE9` |
...
| `transferOwnership` / `acceptOwnership` | 当前 / 待接任 owner | OpenZeppelin 两步交接，通过接任账户确认来降低误交接风险；新 owner 接受后才获得发行和证明更新权限。 |

参数 `amount` 均为最小单位，例如 `mint(to, 1000000)` 表示 **1,000 FCWR**，不是一百万个完整 Token。标准 ERC-20 的零数量转账仍被允许；零数量发行与销毁被本项目主动拒绝。

### 完整合约代码

以下为本次 Fuji 部署使用的合约代码，依赖 OpenZeppelin Contracts 5.0.2。
...
证明文件为 [v1](/Users/bareerah/Development/avalanche/task5-course/learn/BareerahBenjamin/task5/docs/asset-proof-v1.json) 和 [v2](/Users/bareerah/Development/avalanche/task5-course/learn/BareerahBenjamin/task5/docs/asset-proof-v2.json)，链上保存原始文件字节的 `sha256:0x<64位十六进制摘要>`。v2 补充了销毁交易与线下提货申请逐笔对账的说明。
...
完整的交易回执、状态和权限拒绝结果见 [onchain-verification.json](/Users/bareerah/Development/avalanche/task5-course/learn/BareerahBenjamin/task5/evidence/onchain-verification.json)，原始广播记录见 [broadcast.json](/Users/bareerah/Development/avalanche/task5-course/learn/BareerahBenjamin/task5/evidence/broadcast.json)。这两个文件仅保存公开链上信息。非授权行为使用只读 `eth_call` 模拟，不发送无意义的失败交易。
...
**25 项测试全部通过**，其中 1 项 fuzz 测试执行 256 组随机输入。实际日志：[tests.txt](/Users/bareerah/Development/avalanche/task5-course/learn/BareerahBenjamin/task5/evidence/tests.txt)。
...
脚本强制 `block.chainid == 43113`，按顺序部署 → 发行 → 转账 → 销毁 → 更新证明，并检查最终余额。`verify_onchain.py` 固定核对本文记录的部署；若重新部署，需要同步替换脚本的地址和 `evidence/broadcast.json`。

## 8. 截图材料

### 合约部署成功

![部署成功：合约地址、交易哈希与 Success 状态](/Users/bareerah/Development/avalanche/task5-course/learn/BareerahBenjamin/task5/evidence/01-deployment.jpg)

### 测试网合约页面

![Fuji 合约页面：源码验证标记与四笔交互记录](/Users/bareerah/Development/avalanche/task5-course/learn/BareerahBenjamin/task5/evidence/02-contract.jpg)

### Token 发行

ERC-20 发行事件显示从零地址转出 1,000 FCWR；浏览器将此类事件统一显示为 Token Transfer。

![发行 1000 FCWR 成功](/Users/bareerah/Development/avalanche/task5-course/learn/BareerahBenjamin/task5/evidence/03-mint.jpg)

### Token 转账

![向学员钱包转账 200 FCWR 成功](/Users/bareerah/Development/avalanche/task5-course/learn/BareerahBenjamin/task5/evidence/04-transfer.jpg)

### Token 销毁

ERC-20 销毁事件显示向零地址转入 50 FCWR，链上总供应量因此从 1,000 降为 950 FCWR。

![销毁 50 FCWR 成功](/Users/bareerah/Development/avalanche/task5-course/learn/BareerahBenjamin/task5/evidence/05-burn.jpg)
